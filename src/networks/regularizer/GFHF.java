...
public class GFHF {
    ...
    public GFHF(String datasetName, Graph G, HashMap<Integer,String> labels) throws Exception{
        this.labels = labels;
    ...
    public void initialize(){
        ...
        for(int v = 0; v < G.getVertices().size(); v++){
            ...
            for(Edge edge : G.getEdgesFromVert(v)){
                ...
                hm.put(v2, (edge.weight/graus.get(v)));
            }
        }
        ...
        while(control){
            ...
            for(int v : order){
                ...
                for(int v2 : hm.keySet()){
                    for(int classe = 0; classe < qttClasses; classe++){
                        fTemp[v][classe] += f[v2][classe] * hm.get(v2);
                    }
                }
            }
